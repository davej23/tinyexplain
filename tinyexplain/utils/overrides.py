...
def get_model_layers(model: TinygradModel, obj_type: Any) -> dict[str, Any]:
    ...
    layers = [eval(f"{dict((str(v), k) for k, v in locals().items())[str(model)]}.{layer_name}") for layer_name in layer_names]
            ...
            for t, g in zip(t0._ctx.parents, grads):
                if g is not None and t.requires_grad:
                    ...
                    if str(t) in [str(r) for r in RELUS]:
                        grad = (t.grad.relu() * t.relu()) + g.relu() if gbp else t.grad.relu() + g.relu()
