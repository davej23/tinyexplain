...
def count_model_parameters(model: TinygradModel) -> tuple[int, int]:
    layer_info = dict(
        (l.flatten().shape[0], l.requires_grad)
        for l in nn.state.get_state_dict(model).values()
    )
    return sum(
        number_params if requires_grad else 0
        for number_params, requires_grad in layer_info.items()
    ),sum(
        number_params if not requires_grad else 0
        for number_params, requires_grad in layer_info.items()
    )
...
def summary(model: TinygradModel, x: Tensor) -> None:
    ...
    def _tinyexplain_call_override(
        self, x: Tensor
    ) -> Tensor:  # Overwriting layer __call__ to track output shape
    ...
    tinygrad_layers = [
        n[1]
        for n in inspect.getmembers(nn, inspect.isclass)
        if "tinygrad.nn" in str(n[1])
    ]
    ...
    print(
        f"{'output (Tensor)':<110s}{str(out.shape):<20s}{str(out.shape):<20s}{'N/A':<20s}"
    )
