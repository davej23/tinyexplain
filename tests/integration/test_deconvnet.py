...
from tinyexplain.explain import DeconvNet
...
def test_object_detection():
    ...
    explainer = DeconvNet(model, TinyExplainTask.OBJECT_DETECTION)
...
def test_image_classification():
    ...
    explanation = explainer.explain(
        Tensor.rand((1, 3, 28, 28), dtype=dtypes.float32), Tensor.rand((1, 10), dtype=dtypes.float32), lambda x: x
    )
...
def test_tabular_data():
    ...
    explainer = DeconvNet(model, TinyExplainTask.TABULAR_DATA)
...
def test_time_series():
    ...
    explainer = DeconvNet(model, TinyExplainTask.TIME_SERIES)
    explanation = explainer.explain(
