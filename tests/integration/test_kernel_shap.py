...
from tinyexplain.explain import KernelShap
...
def test_object_detection():
    ...
    explainer = KernelShap(model, TinyExplainTask.OBJECT_DETECTION, 10)
...
def test_image_classification():
    ...
    explainer = KernelShap(model, TinyExplainTask.IMAGE_CLASSIFICATION, 10)
    explanation = explainer.explain(
...
def test_tabular_data():
    ...
    explainer = KernelShap(model, TinyExplainTask.TABULAR_DATA, 10)
    explanation = explainer.explain(
...
def test_time_series():
    ...
    explainer = KernelShap(model, TinyExplainTask.TIME_SERIES, 10)
    explanation = explainer.explain(
